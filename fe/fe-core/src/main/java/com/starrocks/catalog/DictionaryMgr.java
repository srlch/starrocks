...
public class DictionaryMgr implements Writable, GsonPostProcessable {
    ...
    private ConcurrentHashMap<Long, Long> dictionaryIdTolastSuccessTxnId = new ConcurrentHashMap<>();
        ...
        try {
            for (Map.Entry<Long, Dictionary> entry : dictionariesMapById.entrySet()) {
                ...
                if (dictionary.getNextSchedulableTime() <= System.currentTimeMillis() &&
                    ...
                    dictionary.updateNextSchedulableTime(dictionary.getRefreshTime());
                }
            }
    ...
    public void addDictionary(Dictionary dictionary) {
        ...
        try {
            ...
            dictionaryIdTolastSuccessTxnId.put(dictionary.getDictionaryId(), 0L);
        } finally {
            lock.unlock();
        }
    }
    ...
    public synchronized void updateLastSuccessTxnId(long dictionaryId, long txnId) {
        dictionaryIdTolastSuccessTxnId.put(dictionaryId, txnId);
    }
    ...
    public long getLastSuccessTxnId(long dictionaryId) {
        return dictionaryIdTolastSuccessTxnId.get(dictionaryId);
    }
    ...
    public void replayCreateDictionary(Dictionary dictionary) {
        dictionary.resetUpdateNextSchedulableTime();
        dictionary.updateNextSchedulableTime(dictionary.getRefreshTime());
        addDictionary(dictionary);
    }
    ...
    public long saveDictionaryMgr(DataOutputStream dos, long checksum) throws IOException {
        write(dos);
        return checksum;
    }
    ...
    public void load(DataInput in) throws IOException {
        ...
        DictionaryMgr data = GsonUtils.GSON.fromJson(json, DictionaryMgr.class);
    ...
    public void gsonPostProcess() throws IOException {
        ...
        try {
            for (Map.Entry<Long, Dictionary> entry : dictionariesMapById.entrySet()) {
                // init for every dictionary
                dictionaryIdTolastSuccessTxnId.put(entry.getKey(), 0L);
            }
        } finally {
            lock.unlock(); 
        }
    ...
    public class RefreshDictionaryCacheWorker implements Runnable {
        ...
        private void finish(long dictionaryId) {
            ...
            } else if (errMsg.indexOf("cancel") != -1 /* cancel by user */) {
                dictionary.resetStateBeforeRefresh();
                dictionary.setErrorMsg("Cancelled Refreshing by user");
            } else {
                dictionary.setCancelled();
                dictionary.setErrorMsg(errMsg);
            }

            dictionary.setStateBeforeRefresh(null); // reset state before refresh
        }
