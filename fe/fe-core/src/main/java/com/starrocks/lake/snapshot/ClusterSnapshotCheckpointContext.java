...
public class ClusterSnapshotCheckpointContext {
    ...
    public synchronized Pair<Long, Boolean> coordinateTwoCheckpointsIfNeeded(boolean belongToGlobalStateMgr,
        ...
        do {
            ...
            if (belongToGlobalStateMgr) {
                ...
                checkpointId = consistentIds.first;
            } else {
    ...
    public void handleImageUpload(Pair<Boolean, String> createImageRet, boolean checkpointIsReady, String imageDir,
        ...
        if (!createImageRet.first && checkpointIsReady) {
            setCurErrMsg(createImageRet.second);
            return;
        }
    ...
    private synchronized void updateRoundIdAndMarkJobFinishedFromLaggard(boolean belongToGlobalStateMgr) {
        ...
        // set FE/starMgr round finished. The order is matter, the flag must be updated after job set finished
        if (belongToGlobalStateMgr) {
