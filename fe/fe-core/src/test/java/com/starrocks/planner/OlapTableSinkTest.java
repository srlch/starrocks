...
public class OlapTableSinkTest {
    ...
    public void testRangeUnknownPartition(
        ...
        OlapTableSink sink = new OlapTableSink(dstTable, tuple, Lists.newArrayList(unknownPartId), TWriteQuorumType.MAJORITY, false);
        sink.init(new TUniqueId(1, 2), 3, 4, 1000);
        sink.complete();
        LOG.info("sink is {}", sink.toThrift());
        LOG.info("{}", sink.getExplainString("", TExplainLevel.NORMAL));
    ...
    public void testSingleListPartition() throws UserException{
        ...
        OlapTableSink sink = new OlapTableSink(dstTable, tuple, Lists.newArrayList(1L), TWriteQuorumType.MAJORITY, false);
        sink.init(new TUniqueId(1, 2), 3, 4, 1000);
        sink.complete();
