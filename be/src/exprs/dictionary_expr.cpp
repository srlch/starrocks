...
StatusOr<ColumnPtr> DictionaryExpr::evaluate_checked(ExprContext* context, Chunk* ptr) {
    ...
    size_t size = ptr != nullptr ? ptr->num_rows() : 0;
    ...
    for (size_t i = 0; i < value_chunk->columns().size(); ++i) {
        ...
        if (nullable_column->is_constant()) {
            auto* cc = ColumnHelper::get_data_column(nullable_column.get());
            fields[i]->append_value_multiple_times(*cc, 0, nullable_column->size());
        } else {
            fields[i]->append(*nullable_column, 0, nullable_column->size());
        }
    }
