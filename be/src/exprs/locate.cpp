...
ColumnPtr haystack_vector_and_needle_const(const ColumnPtr& haystack_ptr, const ColumnPtr& needle_ptr,
    ...
    if (needle.size == 0) {
        ...
        locate_empty_needle_impl(offsets.data(), num_rows, start_data, result_data);
        if (res_null != nullptr) {
    ...
    locate_non_empty_needle_impl(offsets.data(), num_rows, begin, end, needle, start_data, result_data);
