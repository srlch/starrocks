...
Status OlapTableSink::_fill_auto_increment_id(vectorized::Chunk* chunk) {
...
Status OlapTableSink::_fill_auto_increment_id_internal(vectorized::Chunk* chunk, SlotDescriptor *slot, int64_t table_id) {
    ...
    // We reuse vectorized::NullableColumn to represent the auto-increment column because vectorized::NullableColumn
    ...
    ColumnPtr& data_col = std::dynamic_pointer_cast<vectorized::NullableColumn>(col)->data_column();
    std::vector<uint8_t> filter(std::dynamic_pointer_cast<vectorized::NullableColumn>(col)->immutable_null_column_data());
    ...
    case TYPE_BIGINT: {
        ...
        StorageEngine::instance()->get_next_increment_id_interval(table_id, null_rows, ids);
        st = (std::dynamic_pointer_cast<vectorized::Int64Column>(data_col))->fill_range(ids, filter);
        break;
    }
    default:
