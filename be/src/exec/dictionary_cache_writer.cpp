...
Status DictionaryCacheWriter::prepare() {
    ...
    // construct _closures
    _closures.resize(nodes.size());
    for (auto& _closure : _closures) {
        auto closure = new ReusableClosure<PRefreshDictionaryCacheResult>();
        closure->ref();
    ...
    auto st = _schema->init(_t_dictionary_cache_sink.schema);
    if (!st.ok()) {
        return st;
    }
...
Status DictionaryCacheWriter::_send_request(const Chunk* chunk) {
    ...
    for (size_t i = 0; i < nodes.size(); ++i) {
        ...
        auto& closure = _closures[i];
        closure->ref();
        closure->reset();
        closure->cntl.set_timeout_ms(60000);
        closure->cntl.ignore_eovercrowded();
        closure->request_size = request.ByteSizeLong();
...
Status DictionaryCacheWriter::_wait_response() {
    Status st = Status::OK();
    for (size_t i = 0; i < _t_dictionary_cache_sink.nodes.size(); ++i) {
        auto& closure = _closures[i];
        if (closure->join()) {
