            ...
            for (const auto& index : indexes) {
                if (index.index_type() == IndexType::GIN) {
                    ...
                    for (const auto& file : files) {
                        ...
                        auto dst_absolute_path = fmt::format("{}/{}_{}_{}_{}", dir, rowset_id().to_string(),
                                                             i, index.index_id(), file);

                        if (!fs::copy_file(src_absolute_path, dst_absolute_path).ok()) {
                            LOG(WARNING) << "Error to copy index. src:" << src_absolute_path << ", dst:" << dst_absolute_path
                                         << ", errno=" << std::strerror(Errno::no());
