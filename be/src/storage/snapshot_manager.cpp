...
Status SnapshotManager::convert_rowset_ids(const string& clone_dir, int64_t tablet_id, int32_t schema_hash) {
    ...
    for (const auto& file : all_files) {
        if (file.find(".fdt", 0) != std::string::npos ||
            file.find(".fdx", 0) != std::string::npos ||
            file.find(".fnm", 0) != std::string::npos ||
            file.find(".frq", 0) != std::string::npos ||
            file.find(".nrm", 0) != std::string::npos ||
            file.find(".prx", 0) != std::string::npos ||
            file.find(".tii", 0) != std::string::npos ||
            file.find(".tis", 0) != std::string::npos ||
            file.find("null_bitmap", 0) != std::string::npos ||
            file.find("segments_2", 0) != std::string::npos ||
            ...
            RETURN_IF_ERROR(FileSystem::Default()->rename_file(clone_dir + "/" + file, inverted_index_path + "/" + new_file_name));
        }
    }
    ...
    for (const auto& file : all_files) {
        ...
        if (is_dir.ok() && is_dir.value() && file.find("ivt", 0) != std::string::npos) {
            ...
            for (const auto& index_file : index_files) {
                ...
                std::string new_name = snapshot_dir + "/" + rowsetid + "_" + segment_id + "_" + index_id + "_" + index_file;
